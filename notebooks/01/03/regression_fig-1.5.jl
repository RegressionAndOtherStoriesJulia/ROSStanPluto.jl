...
let
	...
	f = scatter(x, y; leg=:topright, lab="simulated values")
	plot!(x1, means[1] .+ means[3] .* x1; lab="Linear regression")
	append!(figs, [f])
	
	f = scatter(x, y; leg=:topright, lab="simulated values")
	plot!(x1, means[2] .+ means[4] .* exp.(-x1); lab="Non-linear regression")
	append!(figs, [f])

	plot(figs...; layout=(2, 1), plot_title="Fig 1.5", titlefontsize=9)
end
