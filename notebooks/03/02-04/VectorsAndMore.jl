...
md"## Vectors, lines and exponential and log relationships"
...
All notebooks expect a pointer to the downloaded ROS-Examples R library. One easy way is to define an enviroment variable for this. For easy access include below cell in each notebook."
...
After evaluating above cell, use `ros_data(\"ElectionsEconomy\", \"hibbs.dat\")` to obtain data."

# ╔═╡ 6c2043f4-dcbd-4535-a5ae-61c109519879
hibbs = CSV.read(ros_data("ElectionsEconomy", "hibbs.dat"), DataFrame; delim=" ")

# ╔═╡ 265a77e8-fddf-442e-9ab0-cab989d3c8d2
let
	fig = plot(;title="Forecasting the election from the economy.")
	scatter!(hibbs.growth, hibbs.vote;
		xlab="Average recent growth in personal income.",
		ylab="Incumbent's party vote share.",
		lab="Vote share", leg=:bottomright
	)
	for i in 1:size(hibbs, 1)
		annotate!(hibbs.growth[i], hibbs.vote[i] + 0.6, (hibbs.year[i], 6))
	end
	fig
end

# ╔═╡ 0d2dabd9-a0fd-462a-84e5-5a192d6aaebb
describe(hibbs)
...
# ╔═╡ 30f85fbc-69d6-480e-88ff-5c8d46a0380d
...
begin
	...
	for i in 1:size(hibbs, 1)
		annotate!(hibbs.growth[i], hibbs.vote[i] + 0.6, (hibbs.year[i], 6, :darkred))
	end
	â = sdf[sdf.parameters .== :a, :mean][1]
	b̂ = sdf[sdf.parameters .== :b, :mean][1]
	plot!(x, â .+ b̂ * x; lab="Regression line")
	annotate!(4, 55, 
		("y = $(round(â; digits=1)) + $(round(b̂; digits=1)) * x",
			10, :darkblue))
end
...
md"#### Compute median and mad_sd."

# ╔═╡ 0625e80c-3106-487d-9821-7e32aba5dc16
(median=median(post1_1s.a), mad=mad(post1_1s.a), std=std(post1_1s.a))

# ╔═╡ 71adb370-6670-4068-82a3-8caf6ab98f12
mad(post1_1s.a; normalize=false)

# ╔═╡ 3b9e883e-c243-4631-9191-13ec528fa032
(median=median(post1_1s.b), mad=mad(post1_1s.b), std=std(post1_1s.b))

# ╔═╡ 4a199052-40cc-4c3c-844c-b04dee21f921
md" ##### ŷ = â + b̂ * x"

# ╔═╡ 23f5b6be-330c-45ea-bdea-bd9563c5b938
let
	fig = plot(;leg=false, xlim=(-2, 4), ylim=(40, 60))
	x = [-1, 0, 3]
	scatter!(x, â .+ b̂ * x)
	for x1 in x
		annotate!(x1 + 0.4, â .+ b̂ * x1, ("$(round(â .+ b̂ * x1; digits=1))"))
	end
	x2 = -2:0.01:4
	plot!(x2, â .+ b̂ * x2)
	vline!([1.22])
...
# ╔═╡ 3c697789-07a4-476b-976e-90f4e8b0bac1
function exp_growth(x; A=1.5e9)
	A *  ℯ ^ (0.014(x - 1900))
end

# ╔═╡ 98c76ef7-efc3-45be-9a41-3b3e59d843dc
exp_growth(1950)

# ╔═╡ c7981c61-cee0-4337-aa73-704302bab68e
log(1.5e9)

# ╔═╡ 3034fdff-c862-4aa6-a4ed-3d85c11214c5
let
	x=1890:1:2040
	plot(x, exp_growth.(x); leg=false, title="Exponential growth")
end

# ╔═╡ 4a811409-125e-443b-aa7d-fa4bf2951e79
function exp_decline(x; A=1000, d=0.8)
	A *  ℯ ^ (x * log(d))
end

# ╔═╡ 2b7773f0-746d-429c-b92e-bbd6ef992a6c
log(0.8)

# ╔═╡ 5092f0e0-6d42-4c89-9693-e4ded1aa6585
let
	x=1:0.1:20
	plot(x, exp_decline.(x); leg=false, title="Exponential decline")
end

# ╔═╡ 2c3c7dd2-f516-4f78-ade4-19b9bcea31b8
	animals = DataFrame(
		animal = ["mouse", "man", "elephant"],
		weight = [0.02, 65, 3000],
		rate = [0.17, 90, 2000])

# ╔═╡ 3186b54d-266c-4913-87e9-389f36fb2343
let
	x = 0:1:3000
	y = 4.1 * x .^ 0.74
	plot(x, y;
	...
	@df animals scatter!(:weight, :rate; lab=:animal)
end

# ╔═╡ f88a0d5b-a66c-4065-ba44-fe38fbab0dde
let
	x = log.(0.01:0.1:3000)
	y = 1.4 .+ 0.74 * x
	fig = plot(x, y;
	...
	animals.log_weight = log.(animals.weight)
	animals.log_rate = log.(animals.rate)
	colors=[:darkred, :green, :darkblue]
	for i in 1:size(animals, 1)
		scatter!([animals.log_weight[i]], [animals.log_rate[i]]; 
			lab=animals.animal[i], marker=:cross,
			markersize=9, color=colors[i])
	end
	fig
end

# ╔═╡ d08d64ca-edd3-4f22-9c19-f8e4c7bffb83
let
	x = log.(0.01:0.1:3000)
	y = 1.4 .+ 0.74 * x
	fig = plot(x, y;
	...
	colors=[:darkred, :green, :darkblue]
	for i in 1:size(animals, 1)
		@df animals scatter!([:log_weight[i]], [:log_rate[i]]; 
			lab=:animal[i], marker=:cross,
			markersize=9, color=colors[i])
	end
	fig
end
