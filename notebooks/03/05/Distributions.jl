...
# ╔═╡ 4ed94362-8370-4de8-a8aa-5f79ba4523f5
let
	density(heights[heights.sex .== "female", :height]; lab="female")
	density!(heights[heights.sex .== "male", :height]; lab="male")
	density!(heights.height; lab="all")
end
...
begin
	x = 55:0.01:70
	plot(;xlims=(50, 80), leg=false)
	plot!(x, pdf.(wdf, x); color=:black)
	x1 = 63.65 - 3 * 2.68:0.01:63.65 - 2 * 2.68
	plot!(x1, pdf.(wdf, x1); fill=true, fillcolor=:grey)
	x1 = 63.65 - 2 * 2.68:0.01:63.65 - 1 * 2.68
	plot!(x1, pdf.(wdf, x1); fill=true, fillcolor=:darkgrey)
	x1 = 63.65 - 1 * 2.68:0.01:63.65 + 1 * 2.68
	plot!(x1, pdf.(wdf, x1); fill=true, fillcolor=:lightgrey)
	x1 = 63.65 + 1 * 2.68:0.01:63.65 + 2 * 2.68
	plot!(x1, pdf.(wdf, x1); fill=true, fillcolor=:darkgrey)
	x1 = 63.65 + 2 * 2.68:0.01:63.65 + 3 * 2.68
	plot!(x1, pdf.(wdf, x1); fill=true, fillcolor=:grey)
	annotate!(73, 0.12, "68%")
	plot!([63.65, 71.5], [0.1, 0.12]; color=:black)
	annotate!(73.9, 0.05, "13.5%")
	plot!([67.5, 72], [0.04, 0.05]; color=:black)
	annotate!(74, 0.01, "2.5%")
	plot!([70, 72.5], [0.004, 0.01]; color=:black)
...
julia_version = "1.8.0-DEV.1416"
manifest_format = "2.0"
project_hash = "730eb20c22e093ebf4ff40f6ed0d1dbc6a1ecf63"
