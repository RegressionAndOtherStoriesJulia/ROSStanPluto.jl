...
begin
	...
	x = collect(LinRange(12.0, 100.0, 1000))
	lines!(x, [fun(v) for v in x])
	vlines!(ax, [optim["x"][1]]; color=:red)
	hlines!(ax, [optim["lp__"][1]]; color=:darkblue, xmin=[0.10], xmax=[0.2])
	annotations!("x̄ = $(optim["x"])", position=(40,100), textsize=15)
	annotations!("fun(x) = $(optim["lp__"])", position=(40,97), textsize=15)
	f
end

# ╔═╡ d201d8d7-dd5c-43f7-9a76-c75acd271690
optim["x"][1]


# ╔═╡ 7a64b2fd-778f-4397-91ae-f72790422875
...
manifest_format = "2.0"
project_hash = "c8458552988fd7a54fb933bda32dfb2b1a30b389"
...
[[deps.StanOptimize]]
deps = ["CSV", "DataFrames", "DelimitedFiles", "Distributed", "DocStringExtensions", "Documenter", "Parameters", "Random", "Reexport", "StanBase", "StanDump", "Statistics", "Test", "Unicode"]
git-tree-sha1 = "41098b54b4dbc46b75384a011bb2018248f2f56a"
