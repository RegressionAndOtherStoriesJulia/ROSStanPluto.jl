...
# ╔═╡ df81dc1f-2526-4682-b01f-057909b13804
...
let
	...
	two = vlines!([ms7_1s[:a, :median] - 2ms7_1s[:a, :mad_sd], ms7_1s[:a, :median] + 2ms7_1s[:a, :mad_sd]]; linewidth=3)

	ax = Axis(f[1, 2]; title="Density :b", subtitle="+/- 1 std err = blue, +/- 2 std err = yellow")
	hist!(post7_1s.b; bins=15, color = :lightgrey, strokewidth = 1, strokecolor = :grey)
	...
	Legend(f[1, 3], [one, two], ["+/- one std", "+/- tow std"])
	f
end
...
manifest_format = "2.0"
project_hash = "955fd1a5bdf453dff8a9e1154bb4cb3d312bd78b"
