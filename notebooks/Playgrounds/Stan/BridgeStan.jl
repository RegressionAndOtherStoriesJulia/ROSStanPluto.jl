...
let
	...
	DataFrame(x=x, q=q, log_density=lp, gradient=grad)
end
...
# ╔═╡ feda7416-1195-489f-991d-8a876507db45
function sim(smb::BridgeStan.StanModel, x=LinRange(0.1, 0.9, 100))
    ...
    for (i, p) in enumerate(x)
        ...
        ld[i], g[i] = BridgeStan.log_density_gradient(smb, q[i:i], jacobian = 0)
    end
    return DataFrame(x=x, q=q, log_density=ld, gradient=[x[1] for x in g])
end
...
let
	...
	scatter!(x, fm[1])
	lines!([x, x], [fm[1], minimum(sim_df.log_density)]; color=:grey, linestyle=:dash)
	annotations!("$([round(x; digits=2), round(fm[1]; digits=2)])"; position=(0.31, -6))
	gra = lines!(sim_df.x, sim_df.gradient)
	Legend(f[1, 2], [dens, gra], ["log_density", "gradient"])
	f
end

# ╔═╡ e936c5c3-f5c7-423d-846b-529f77499b34
sim_df[findmax(sim_df.log_density)[2], :]

# ╔═╡ 9fb10ce6-9ea3-4857-bc4d-5b139d27eb06
md" ##### Try a simple model."
...
parameters {
	real a;              // Coefficient independent variable
...
let
	N = 100
	x = rand(Normal(2, 1), N)
	y = 2 * x + rand(Normal(0, 1), N)
	data = (N = 100, x = x, y = y)
	global m1_2s = SampleModel("m1_2s", stan1_2)
	global rc1_2s = stan_sample(m1_2s; data)
	if success(rc1_2s)
		global sum1_2s = read_summary(m1_2s)
	end
end
...
let
    ...
    q = [2.0, 0.0] # unconstrained scale
    lp, grad = BridgeStan.log_density_gradient(smb1_2s, q, jacobian = 0)
	DataFrame(q=q, log_density=lp, gradient=grad)
end
...
if success(rc1_1s)
	sum1_1s = read_summary(m1_1s)
end

# ╔═╡ 1503a0c8-20ff-4821-8dc8-7299617628ef
sum1_1s[9, :mean]

# ╔═╡ e5129e29-9281-4796-88b6-020d9e8d580c
begin
	smb1_1s = create_smb(m1_1s, joinpath(m1_1s.tmpdir, m1_1s.name * "_data_4.json"))
    q = rand(BridgeStan.param_unc_num(smb1_1s)) # unconstrained scale
    lp, grad = BridgeStan.log_density_gradient(smb1_1s, q, jacobian = 0)
	DataFrame(q=q, log_density=lp, gradient=grad)
end

# ╔═╡ 0c17b7d0-cb6e-4ccc-8685-1fa962aebebf
let
    qmax = [sum1_1s[8, :mean], sum1_1s[9, :mean], 1.03] # unconstrained scale
    lp, grad = BridgeStan.log_density_gradient(smb1_1s, qmax, jacobian = 0)
	DataFrame(q=qmax, log_density=lp, gradient=grad)
end

# ╔═╡ 9c82419f-3577-46a4-b759-2badbcd1411f
mean(rand(Exponential(1), 1000))

# ╔═╡ 450c557d-5e11-4061-b807-fffb429212f7
BridgeStan.param_unc_num(smb1_1s)

# ╔═╡ 7a709a1a-e8b1-4c3f-abcb-e3facd87fee9
BridgeStan.param_names(smb1_1s)

# ╔═╡ 4b433a67-2a29-41c2-a04a-83572eb09dd2
let
	smb1_1s = create_smb(m1_1s, joinpath(m1_1s.tmpdir, m1_1s.name * "_data_4.json"))
    q = [3.04, 43.32, 1.0] # unconstrained scale
    lp, grad = BridgeStan.log_density_gradient(smb1_1s, q, jacobian = 0)
	DataFrame(q=q, log_density=lp, gradient=grad)
end

# ╔═╡ 79aa12f4-abef-479a-8644-25a9f5588f22
function sim2(smb1_1s::BridgeStan.StanModel, x=LinRange(1, 6, 100))
    q = Vector{Vector{Float64}}(undef, length(x))
    ld = zeros(length(x))
    g = Vector{Vector{Float64}}(undef, length(x))
    for (i, p) in enumerate(x)
        q[i] = [x[i], 43.32, 1.0] # unconstrained scale
        ld[i], g[i] = BridgeStan.log_density_gradient(smb1_1s, q[i],
            jacobian = 0)
    end
    return DataFrame(x=x, q=q, log_density=ld, gradient=[x[1] for x in g])
end

# ╔═╡ 3648ec90-7fda-4047-b0dc-0c448f795431
@time sim_df_2 = sim2(smb1_1s)

# ╔═╡ 52c42c16-03f7-4416-9180-835b7ed9d942
sim_df_2[findmax(sim_df_2.log_density)[2], :]

# ╔═╡ dca12cf4-6d3c-4899-8b70-c7d974705064
sim_df_2[59:70, :]
...
manifest_format = "2.0"
project_hash = "c70379a742a24b77cee1156d84719e727186eb75"
