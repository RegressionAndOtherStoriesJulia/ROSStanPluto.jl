...
md"##### See Chapter 1.3 in Regression and Other Stories."
...
begin
	...
	# Access to ROSStanPluto repo data dir
	datadir(parts...) = normpath(joinpath(@__DIR__), "../../../../data", 
		parts...)
end;

# ╔═╡ 4f8d9c53-c073-4838-b3cf-9b72df9f7d70
penguins = CSV.read(datadir("PalmerPenguins", "palmerpenguins.csv"),
	DataFrame)

# ╔═╡ fbaedcea-922a-4097-a0ae-53a9cfb04c61
begin
	set_aog_theme!()
	axis = (width = 225, height = 225)
end;

# ╔═╡ a64dcc0b-ea9d-4269-9df2-d1032a72fe2b
penguin_frequency = data(penguins) * frequency() * mapping(:species)

# ╔═╡ 6cb9c7aa-a579-406b-b2e8-fd6b73f33b1e
draw(penguin_frequency; axis)

# ╔═╡ da9890c1-6fc8-4d5b-934f-6d9e0e509627
md" #### Saving a plot."

# ╔═╡ 5e04334d-f693-4f61-8cd5-681ea6d6e534
let
	fg = draw(penguin_frequency; axis)
	# save high-resolution png
	save(joinpath(@__DIR__, "figure.png"), fg, px_per_unit = 3)
end;

# ╔═╡ 332e8b6a-1c9c-474b-88d9-eef4ba5a48f1
let
	plt = penguin_frequency * mapping(color = :island)
	draw(plt; axis)
end

# ╔═╡ 663428e4-ad40-4cec-af38-5a9ac501ca4c
let
	plt = penguin_frequency * mapping(color = :island, dodge = :island)
	draw(plt; axis)
end

# ╔═╡ 662782f5-3a2f-49a9-b2e8-cf62672fa538
let
	plt = penguin_frequency * mapping(color = :island, stack = :island)
	draw(plt; axis)
end

# ╔═╡ d14ab999-383e-4099-8590-8393783a3162
md" #### Correlating two variables."

# ╔═╡ b9d9d98b-e4ee-4ae2-8746-9a6335aa0abc
let
	penguin_bill = data(penguins) * mapping(:bill_length_mm, :bill_depth_mm)
	draw(penguin_bill; axis)
end

# ╔═╡ 536983ca-de06-49f2-96b5-cb6df59897f8
begin
...
# ╔═╡ 29578af7-65a5-4a7d-a6b1-7a0b07a784f3
let
	plt = penguin_bill * mapping(color = :species)
	draw(plt; axis)
end

# ╔═╡ de0d4535-8747-4ddc-a17c-f58b184c50ac
let
	plt = penguin_bill * linear() * mapping(color = :species)
	draw(plt; axis)
end

# ╔═╡ 131f6b34-dd07-40dd-aa4d-e59a7192b1c1
let
	plt = penguin_bill * linear() * mapping(color = :species) + 
...
# ╔═╡ da7429ab-e01f-4cca-bd9f-16a89ca591a1
let
	plt = penguin_bill * (linear() + mapping()) * mapping(color = :species)
	draw(plt; axis)
end

# ╔═╡ 57752f65-f6bd-489d-8b88-137474ff60c1
let
...
# ╔═╡ 390509c1-5998-4da1-9a4b-6bdfdd9cbd63
let
...
# ╔═╡ 0825c825-9970-4258-a0c7-c2ca35ebd3dc
let
...
# ╔═╡ 4357fde1-9173-442b-b3ba-6be51e1497ca
let
	layers = linear() + mapping()
...
# ╔═╡ 699febca-7fbe-45ed-9187-caed04daa5a6
md" #### Smooth density plots."

# ╔═╡ ab38836b-8100-42c9-9124-8c5b1174b7d4
let
	plt = penguin_bill * density(npoints=50) * mapping(col = :species)
	draw(plt; axis)
end

# ╔═╡ 8099c302-5c8b-4c28-87a3-1cff6145f26d
let
	plt = penguin_bill * density(npoints=50) * mapping(col = :species) *
		visual(colormap = :grayC)
	draw(plt; axis)
end

# ╔═╡ 1f8bcefd-8eac-4ba1-b9c4-fc4c76d230d5
let
	axis = (type = Axis3, width = 300, height = 300)
	layer = density() * visual(Wireframe, linewidth=0.8)
	plt = penguin_bill * layer * mapping(color = :species)
	draw(plt; axis)
end

# ╔═╡ 794048db-a3bf-4872-b5c1-7581e53a044f
let
	axis = (width = 225, height = 225)
...
# ╔═╡ fc2af0c3-68ec-4263-8d62-f8a72be853c6
let
...
# ╔═╡ 7ae514c6-6531-45c9-bb55-ec01245122b6
let
...
# ╔═╡ 78afa031-2dd0-40af-8a39-524905b51657
md" #### Correlating three variables."

# ╔═╡ d11a64a0-69a0-4dc8-b163-6d2494b4376d
let
	global body_mass = :body_mass_g => (t -> t / 1000) => "body mass (kg)"
	layers = linear() * mapping(group = :species) + mapping(color = body_mass,
		marker = :species)
	plt = penguin_bill * layers
	draw(plt; axis)
end

# ╔═╡ a522a096-c688-4841-9392-0512a9949f3c
let
	axis = (type = Axis3, width = 400, height = 300)
	plt = penguin_bill * mapping(body_mass, color = :species) *
		visual(Scatter, markersize = 30)
	draw(plt; axis)
end

# ╔═╡ 5fd7d8d8-6cf6-4f70-82db-22262fc789c4
let
...
git-tree-sha1 = "0ad37706160da3c5a08b6429f2b4bc078abc479d"
...
[[StanSample]]
deps = ["CSV", "DataFrames", "DelimitedFiles", "Distributed", "DocStringExtensions", "MonteCarloMeasurements", "NamedTupleTools", "OrderedCollections", "Parameters", "Random", "Reexport", "Requires", "StanBase", "StanDump", "TableOperations", "Tables", "Unicode"]
git-tree-sha1 = "4f1521585e5a93cdb1d5abf80427527837f952d9"
