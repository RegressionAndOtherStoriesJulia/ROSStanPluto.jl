...
Most notebooks expect a pointer to the downloaded ROS-Examples R library. One easy way is to define an enviroment variable for this. For easy access include below cell in each notebook.

Over time I might add all data files as .csv files to the local data directory."
...
let
	...
	let
		plt = data(post1_1s_df) * mapping(:a; color=:chain) * density()
		axis = (; title="Density")
	...
	let
		plt = data(post1_1s_df) * mapping(:b; color=:chain) * density()
		axis = (; title="Density")
...
begin
	...
	let
		...
		plt = data(hibbs) * 
			...
			visual(Annotations, textsize=10)
		axis = (; title, xlabel, ylabel)
		draw!(fig[1, 1], plt; axis)
	end

	# Superimpose Stan fit (red dashes) on previous Linear() plot
	let
		global ā = sdf[sdf.parameters .== :a, :mean][1]
		global b̄ = sdf[sdf.parameters .== :b, :mean][1]
		title = "Stan fit ($(round(ā, digits=1)) + $(round(b̄, digits=1)) * \
			growth)"
		axis = (; title, xlabel, ylabel)
		df = DataFrame()
		df.x = 0:0.01:4
		...
		plt2 = data(hibbs[[1, 6, 7, 15, 16], :]) * 
			mapping(:label => verbatim, (:growth, :vote) => Point) *
			visual(Annotations, textsize=10)
		plt3 = data(df) * mapping(:x, :y) *
			visual(Lines, color=:red, linestyle=:dash, linewidth=3)
		layers = plt1 + plt2 + plt3
		draw!(fig[1, 2], layers; axis)
	end
	fig
end

# ╔═╡ 5b6ad338-f977-4ddb-8d98-25375ff1e58a
let
	f = Figure()
	df = DataFrame()
	df.x = 0:0.01:4
	...
	plt3 = data(df) * mapping(:x, :y) *
		visual(Lines, color=:red, linestyle=:dash, linewidth=3)
	layers = plt1 + plt2 + plt3
	draw!(f[1, 1], layers; axis)
	f
end

# ╔═╡ eaf56532-9690-4487-ae86-10b92d8ea4b1
plt3
...
# ╔═╡ 54c33c18-cc31-472f-93c9-84989a261414
post1_1s
...
md" #### Priors and draws (100 lines)."

# ╔═╡ 80595ea6-daaf-4a52-80dc-3f458cc261ba
let
	N = 100
	x = LinRange(1, 4, N)
	...
	fig = Figure()
	title = "Priors (100 lines)"
	ax1 = Axis(fig[1, 1]; title, xlabel, ylabel)
	lines!(fig[1, 1], x, a[1] .+ b[2] * x, color = :lightgrey)
	for i in 2:N
		y = a[i] .+ b[i] * x
		lines!(fig[1, 1], x, y, color = :lightgrey)
	end
	text!("First 100 a and b samples"; position=(1, -100))

	ā = sdf[sdf.parameters .== :a, :mean][1]
	b̄ = sdf[sdf.parameters .== :b, :mean][1]
	title = "Posterior draws (100 lines)"

	ax2 = Axis(fig[1, 2]; title, xlabel, ylabel)
	for i in 2:100
		y = post1_1s_df.a[i] .+ post1_1s_df.b[i] * x
		lines!(fig[1, 2], x, y, color = :lightgrey)
	end
	annotations!("vote = 46 + 3 * growth"; position=(1.5, 47))
	lines!(fig[1, 2], x, ā .+ b̄ * x, color = :red)
	
	fig
end

# ╔═╡ b17d80c0-b662-4689-8ebf-59fef0f0befe
md" #### Mix and match Makie and AlgebraOfGraphics?"

# ╔═╡ 5ec5dd50-ed64-4122-8262-37bde61fe003
let
...
# ╔═╡ 7f05bef1-1227-46f5-80ea-6af60cdcdabe
let
	df = (x = exp.(rand(1000)),)
	hist1 = data(df) * mapping(:x => log => "log(x)") * histogram()
	fg = draw(hist1; figure = (resolution = (600,400),))
...
julia_version = "1.8.0-DEV.1455"
manifest_format = "2.0"
project_hash = "e81c3ecd56b93558ec39f57203c85f330b281d3d"
...
git-tree-sha1 = "c672468fdd83d96414279a223ef655911c912e6c"
...
git-tree-sha1 = "6d497458dc429087217e82f268ec35884018fc45"
