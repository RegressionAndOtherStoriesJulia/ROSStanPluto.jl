...
# ╔═╡ 09ccd0a0-88f9-46f8-a714-6bf86baf6947
using Pkg, DrWatson

# ╔═╡ 1496b8d1-b50d-4847-aa50-c2d00b047c97
begin
...
# ╔═╡ b1abab42-9391-47f3-afd1-5996afd94728
begin
	mpg = dataset("ggplot2", "mpg")
	mpg[!, :Cyl] = categorical(mpg.Cyl)
end;

# ╔═╡ c89877df-2e85-46e9-b58c-50986a1debb0
begin
	cols = mapping(:Displ, :Hwy)
	grp = mapping(color = :Cyl)
	scat = visual(Scatter) * grp
	pipeline = cols * scat
	data(mpg) * pipeline |> draw
end

# ╔═╡ 6ccd715c-8de4-47d0-a7eb-b3674c3688a8
let
	df = (x=randn(1000), c=rand(["a", "b"], 1000))
	global plt = data(df) * mapping(:x, color=:c) * density(bandwidth=0.5)
	plt |> draw
end

# ╔═╡ 6989dea5-003a-420a-b208-fd38fb9559ec
x, y, df, xy = let
...
# ╔═╡ e1bd5b7c-a982-42df-8479-8b03ee176953
let
	layers = visual(Lines)
	draw(layers * xy)
end

# ╔═╡ 32e417df-80ad-41f7-a006-6565b6912930
let
	layers = visual(Scatter) + visual(Lines)
	layers * xy |> draw
end

# ╔═╡ 0512578e-ceb7-4335-8476-55828bdd5601
let
	df2 = (x=rand(10), y=rand(10))
	layers = data(df) * visual(Lines) + data(df2) * visual(Scatter)
	draw(layers * mapping(:x, :y))
end

# ╔═╡ 2255ff0c-3d90-41d0-9efd-eb87554f52fa
let
...
# ╔═╡ eaccf7ca-5f27-4f0a-b68b-9b8928c9376e
let
	df = (x=rand(100), y=rand(100), i=rand(["a", "b", "c"], 100),
		j=rand(["d", "e", "f"], 100))
	plt = data(df) * mapping(:x, :y, col=:i, row=:j)
	draw(plt)
end

# ╔═╡ afd60b59-e8a1-438a-80d3-260f49ec5de2
let
...
# ╔═╡ 8a15fa6e-afbc-4b32-9d27-c3009d597cd8
let
	df = (x=rand(100), y=rand(100), i=rand(["a", "b", "c"], 100), 
		j=rand(["d", "e", "f"], 100))
	df2 = (x=[0, 1], y=[0.5, 0.5], i=fill("a", 2), j=fill("e", 2))
	layers = data(df) * visual(Scatter) + data(df2) * visual(Lines)
	draw(layers * mapping(:x, :y, col=:i, row=:j))
end

# ╔═╡ ba0c1cfb-5b72-467b-906b-93d2c12531b8
let
	df = (x=randn(1000), c=rand(["a", "b"], 1000))
...
# ╔═╡ 995256d6-851f-4e69-88c0-4699f68ea1d1
let
	x = 1:100
	y = @. sqrt(x) + 20x + 100
	df = (; x, y)
...
# ╔═╡ a403b25a-f189-46eb-a898-ce7b9f89f488
layer = let
	n = 100
	df = (;
			x=rand(n),
	...
	d = data(df)
	m = mapping(:x, color=:c)
	p1 = mapping(:y => :val) * visual(Scatter)
	p2 = mapping(:z => :val) * visual(Lines)
	layers = d*m*(p1 + p2)
	draw(layers)
end

# ╔═╡ 4f791a14-ae75-462c-a1ed-86547f598703
let
	f = Figure()
...
# ╔═╡ f6e2cba7-0a8d-4f23-914b-e8ee57b68ada
fig, x1, y1 = let
...
# ╔═╡ cf0338fa-1137-44ca-8dc6-f71ea2f2f876
fig
...
julia_version = "1.8.0-DEV.1463"
manifest_format = "2.0"
project_hash = "2fea9b5e565d3149f514a18de48091a472f0d22f"
...
git-tree-sha1 = "bd73dee544b5881698456f9283117a5fdab77762"
