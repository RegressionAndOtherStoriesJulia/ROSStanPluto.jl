...
model {
	...
	y ~ normal(a[2] + b[2] * x_binary, sigma[1]);
}
";
...
begin
	...
	#Random.seed!(123)
	df = DataFrame()
	df.x = rand(Uniform(1, 5), n)
	df.x_binary = [df[i, :x] < 3 ? 0 : 1 for i in 1:n]
	df.y = [rand(Normal(10 + 3df[i, :x], 3), 1)[1] for i in 1:n]
	nt = (N = n, x = df.x, x_binary = df.x_binary, y = df.y)
	df
end
...
begin
	...
	rc1_2 = stan_sample(m1_2s; data=nt)
end;

# ╔═╡ 20dea214-9997-4a6e-8d15-7a2bc42d33b6
if success(rc1_2)
	df1_2s = read_samples(m1_2s, :dataframe)
	means = mean(Array(df1_2s); dims=1)
end
...
let
	...
	x1 = 1.0:0.01:5.0
	f1 = scatter(x, y; leg=:bottomright)
	plot!(x1, means[1] .+ means[3] .* x1)
	append!(figs, [f1])

	x2 = 0.0:0.01:1.0
	f2 = scatter(x_binary, y; leg=:bottomright)
	plot!(x2, means[2] .+ means[4] .* x2)
	append!(figs, [f2])
