...
# ╔═╡ 91b470ee-fb71-47d1-99c9-8510696a0c3b
stan6_7 = "
data {
  int <lower=1> N;
  vector[N] h0;
...
# ╔═╡ 08d09e88-a978-4798-a993-a09bedf6fbfa
stan6_8 = "
data {
  int <lower=1> N;
  vector[N] h0;
...
# ╔═╡ d4c2e24d-8174-4af5-bef1-0ca9a72dbf23
begin
	m6_7s = SampleModel("m6.7", stan6_7)
	rc6_7s = stan_sample(m6_7s; data=data)
end;

# ╔═╡ 26f0ff9d-4f34-4924-b884-fe398f80244b
if success(rc6_8s)
  part6_8s = read_samples(m6_8s, :particles);
end

# ╔═╡ bc4f3064-a155-449b-9993-eac8e747a7b9
if success(rc6_7s)
  part6_7s = read_samples(m6_7s, :particles);
end

# ╔═╡ 11640d4b-0107-4c3c-ae71-89697127f2db
if success(rc6_8s)
	post6_8s_df = read_samples(m6_8s, :dataframe)
	post6_8s_df[!, :chain] = repeat(collect(1:m6_8s.num_cpp_chains);
		inner=m6_8s.num_samples)
	post6_8s_df[!, :model] = repeat([1]; inner=size(post6_8s_df, 1))
	post6_8s_df
end

# ╔═╡ 47f67345-747a-42eb-928b-6c934da00a89
if success(rc6_7s)
	post6_7s_df = read_samples(m6_7s, :dataframe)
	post6_7s_df[!, :chain] = repeat(collect(1:m6_7s.num_cpp_chains);
		inner=m6_7s.num_samples)
	post6_7s_df[!, :model] = repeat([2]; inner=size(post6_7s_df, 1))
	post6_7s_df
end

# ╔═╡ 1903a3d9-a2d0-43e5-bdb0-6b77ab538530
begin
	post_df = vcat(post6_8s_df[:, [:a, :bt, :sigma, :chain, :model]],
			post6_7s_df[:, [:a, :bt, :sigma, :chain, :model]])
	post_df[!, :model] = categorical(post_df.model)
end;

# ╔═╡ 57e45f9c-e866-4175-8d08-9a80c09a63a5
let
	fig = Figure()
	
	let
		plt = AlgebraOfGraphics.data(post_df) * visual(Violin) *
			mapping(:model, :a, color=:model)
		axis = (; title="a")
  		draw!(fig[1, 1], plt; axis)
	end
	let
		plt = AlgebraOfGraphics.data(post_df) * visual(Violin) *
			mapping(:model, :bt, color=:model)
		axis = (; title="bt")
  		draw!(fig[1, 2], plt; axis)
	end
	let
		plt = AlgebraOfGraphics.data(post_df) * visual(Violin) *
			mapping(:model, :sigma, color=:model)
		axis = (; title="sigma")
  		draw!(fig[1, 3], plt; axis)
	end
	fig
end

# ╔═╡ c6fa5e59-3fcc-4ea6-bc6a-d903de78a882
md" #### End of coefficient example"
...
[compat]
AlgebraOfGraphics = "~0.6.2"
CategoricalArrays = "~0.10.2"
DataFrames = "~1.3.2"
Distributions = "~0.25.44"
DrWatson = "~2.9.1"
GLMakie = "~0.5.2"
Makie = "~0.16.3"
MonteCarloMeasurements = "~1.0.6"
...
julia_version = "1.8.0-DEV.1436"
manifest_format = "2.0"
project_hash = "9128e86e09fdea1f828de5c76faea846cf678d15"
...
git-tree-sha1 = "591b968e72f15fbe4e33d055db3ff13eb83f1dfc"
