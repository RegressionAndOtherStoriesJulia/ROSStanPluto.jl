...
begin
	...
	using AlgebraOfGraphics: smooth, frequency
...
begin
	...
	scat = visual(Scatter)
	grp = mapping(color = :Cyl)
end;

# ╔═╡ 0479aad0-2a61-4459-bfb7-f6d94643d97e
let
	global pipe = cols * scat
	data(mpg) * pipe |> draw
end

# ╔═╡ b4bdbd76-a806-4d40-9129-9a85789ca14b
data(mpg) * grp * pipe |> draw

# ╔═╡ dc963efe-3951-475f-997c-2d5511c95d78
let
	pipe = cols * (scat + linear())
	data(mpg) * pipe |> draw
end

# ╔═╡ a8b01082-6cf9-4fe8-ac64-5a15f42d5e38
let
	pipe = cols * (scat + linear())
	#data(mpg) * grp * pipe |> draw
	data(mpg) * pipe |> draw
...
# ╔═╡ b08e9866-8bd4-45b7-b756-30aafab6c427
let
	different_grouping = grp * scat + linear() * mapping(wts=:Hwy)
	data(mpg) * cols * different_grouping |> draw
end

# ╔═╡ 8baef71a-77b3-467b-8739-1143b644c80f
let
	data(mpg) * cols * grp * (scat + smooth(span = 0.8)) |> draw
end

# ╔═╡ f9b3ece8-5e37-4d93-877b-88c8e296e71c
let
	newmapping = mapping(markersize = :Cyl) * visual(markersize = (0.1, 5))
	data(mpg) * cols * (scat * newmapping + smooth(span = 0.8)) |> draw
end

# ╔═╡ 9c6e96f0-cb43-49d2-acd0-1b82dcae3ebe
data(mpg) * cols * density() |> draw

# ╔═╡ de351cd7-82a7-4e71-ba25-a6c4490233c1
let
	mpg.IsAudi = mpg.Manufacturer .== "audi"
	geom = visual(BoxPlot)
	data(mpg) *
		mapping(:Cyl, :Hwy) *
		mapping(dodge = :IsAudi, color = :IsAudi) *
		geom |> draw
end

# ╔═╡ 916624c2-6bac-46e3-8882-2e75b1d0f617
begin
	set_aog_theme!()
	axis = (width = 400, height = 350)
end;

# ╔═╡ c0cac314-dc56-4942-9212-c4d9002f67ec
let
	axis = (width = 600, height = 350)
	geom = visual(Violin)
	data(mpg) *
		mapping(:Cyl, :Hwy) *
		mapping(dodge = :IsAudi, color = :IsAudi) *
...
# ╔═╡ 8facaf98-d4a5-49ee-b4b4-868b32e66162
let
	data(mpg) *
		mapping(:Cyl, :Hwy) *
		mapping(dodge = :IsAudi, color = :IsAudi) *
...
# ╔═╡ ecb5e65e-e2f8-4f08-8b94-7fa29df6de6d
iris = dataset("datasets", "iris")

# ╔═╡ b2d5f4c2-34d8-4691-b6bd-7e721510ae70
let
	cols = mapping(:SepalLength, :SepalWidth)
	grp = mapping(color = :Species)
	geom = visual(Scatter) + linear()
	data(iris) * cols * grp * geom |> draw
end

# ╔═╡ 580d888e-e0f4-402c-bc6f-b4accb179977
let
	axis = (width = 500, height = 500)
	cols = mapping(:SepalLength, :SepalWidth)
	layers = linear() + mapping(color = :Species)
	plt = data(iris) * cols * layers * mapping(col = :Species)
	draw(plt; axis)
end
...
git-tree-sha1 = "68b4f97ed148cf6caef4414576bc2d58d91bc539"
...
git-tree-sha1 = "fdf22e6ee3b9bee9108f355592d887d10fda1d32"
